		...
		<div id='tab-0' class='tab-content'>
			<form action='' method='POST' enctype='multipart/form-data'>
				...
				<tbody>
					...
					<tr><td>dotstudioPRO Account Dashboard<br/><span class='description'>Opens in a new window</span></td><td><a class="button" href="https://www.dotstudiopro.com/user/login" target="_blank">LOGIN</a></td></tr>
					<tr><td>dotstudioPRO API Key<br/><span class='description'>Don't have an API Key? <a href="https://beta.dotstudiopro.com/user/register" target="_blank">Click Here.</a></span></td><td><input type='text' name='ds_api_key' value='<?php echo get_option('ds_api_key') ?>' /></td></tr>
					<tr><td>Re-sync and Rebuild<br/><span class='description'>Resync video, channel and category data from the dotstudioPRO dashboard</span></td><td><a class='button' href='<?php echo site_url().'/wp-admin/admin.php?page=dot-studioz-options&flush=1'; ?>'>RE-SYNC</a></td></tr>
					<tr><td>Facebook App ID<br/><span class='description'>For Facebook sharing and commenting to work properly you need a Facebook App Id. Don't have one? <a href="http://dotstudiopro.com" target="_blank">Click Here.</a></span></td><td><input type='text' name='ds_fb_app_id' value='<?php echo get_option('ds_fb_app_id') ?>' /></td></tr>
					...
					<tr><td>Mute Audio On Page Load<br/><span class='description'>Check to mute audio when autoplay is enabled and videos load</span></td><td><input type='checkbox' name='ds_player_mute' value='1' <?php echo get_option("ds_player_mute") == 1 ? 'checked="checked"' : '' ?> /></td></tr>
					...
					<tr><td>Theater Mode Playlist Location<br/><span class='description'>Check to display above video meta information.  Leave unchecked to display below video</span></td><td><input type='checkbox' name='ds_show_playlist_above_meta' value='1' <?php echo get_option("ds_show_playlist_above_meta") == 1 ? 'checked="checked"' : '' ?> /></td></tr>
					...
					<tr <?php if(ds_templates_exist()) { ?>style='background-color:yellow;' <?php } ?>><td>Copy plugin template files to my theme folder <?php if(ds_templates_exist()) { ?>(USE WITH CAUTION)<?php } ?></b><br/><span class='description'>For custom template changes.</span></td><td><a class='button <?php if(ds_templates_exist()) { ?>warn-templates-exist <?php } ?>'  data-href='<?php echo site_url().'/wp-admin/admin.php?page=dot-studioz-options&templatecopy=1'; ?>'>Copy</a></td></tr>
